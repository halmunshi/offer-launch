  ...
  return (
    <section className="animate-fade-up w-full space-y-5">
      <div className="flex items-center justify-between gap-3">
        ...
        <Badge
          variant="outline"
          className="h-7 rounded-pill border-[#dbd8d2] bg-[#f7f5f2] px-3 text-xs font-medium text-[#94908a]"
        >
          {isLoading ? "Loading..." : `${filteredOffers.length} offers`}
        </Badge>
      </div>

      <div className="w-full">
        <div className="grid grid-cols-1 gap-2.5 md:grid-cols-[minmax(0,1fr)_auto_auto_auto]">
          <div className="relative">
            ...
            <Input
              ...
              className="h-9 rounded-[12px] border border-[#d8d4ce] bg-transparent pl-9 text-[15px] placeholder:text-[#a39f98]"
            />
          </div>

          <Select value={sortBy} onValueChange={(value) => setSortBy(value as SortBy)}>
            <SelectTrigger className="h-9 min-w-[154px] rounded-[12px] border border-[#d8d4ce] bg-transparent text-[15px] text-[#3d3a36]">
              <SelectValue placeholder="Sort by" />
            </SelectTrigger>
            <SelectContent>
              <p className="px-2 py-1 text-xs font-semibold uppercase tracking-[0.06em] text-[#8f8a83]">Sort By</p>
              <Separator className="my-1 bg-[#ece9e4]" />
          ...
          <div>
            <Select value="all_creators" onValueChange={() => {}}>
              <SelectTrigger className="h-9 min-w-[174px] rounded-[12px] border border-[#d8d4ce] bg-transparent text-[15px] text-[#807b74] opacity-100">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all_creators" disabled>
                  All creators
                </SelectItem>
                <SelectItem value="current_user" disabled>
                  {user?.fullName ?? user?.firstName ?? "Current user"}
                </SelectItem>
                <Separator className="my-1 bg-[#ece9e4]" />
                <p className="px-2 py-1 text-[11px] text-[#9b968f]">Creator filter coming soon</p>
              </SelectContent>
            </Select>
          </div>
      ...
      <div className="w-full">
        {isLoading ? (
          ...
          showNoOffersState ? (
            ...
            <div className="px-6 py-12 text-center">
              <p className="text-sm font-semibold text-primary">No matching offers</p>
              ...
              <div className="mt-4 flex items-center justify-center">
                <button
                  ...
                  className="inline-flex h-10 items-center rounded-[10px] border border-[#d8d4ce] bg-[#fff] px-4 text-sm font-medium text-[#625e58] transition-colors hover:text-primary"
                >
                  Clear filters
                </button>
              </div>
          ...
          <Link
            href="/offers/new"
            className="group block rounded-[10px] border-2 border-dashed border-[#d1cdc7] bg-transparent transition-colors hover:border-[#bdb7af]"
          >
            <Card className="h-full min-h-[220px] justify-center rounded-[10px] bg-transparent py-0 shadow-none ring-0">
              <Plus className="mx-auto h-8 w-8 text-[#aaa59e] transition-colors group-hover:text-[#8f8a83]" />
